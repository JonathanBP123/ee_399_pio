...
#include <jonathanbp123-project-1_inferencing.h>
...
static bool microphone_inference_start(uint32_t n_samples)
{
    ...
    bool mic_success = mic_setup();
    if (!mic_success) {Serial.println("Mic setup failed.");}
...
bool ei_result(float percent_pass)
{
    ...
    if (result.classification[0].value > percent_pass) {
        return true;
    }
    else {
        return false;
